	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="保险名称" prop="insurance_name">
							  ...
							  v-if="user_group === '管理员' || (form['individual_payment_id'] && $check_field('set','insurance_name')) || (!form['individual_payment_id'] && $check_field('add','insurance_name'))" :disabled="disabledObj['insurance_name_isDisabled']"></el-input>
				...
				<el-form-item label="保险类型" prop="insurance_type">
							  ...
							  v-if="user_group === '管理员' || (form['individual_payment_id'] && $check_field('set','insurance_type')) || (!form['individual_payment_id'] && $check_field('add','insurance_type'))" :disabled="disabledObj['insurance_type_isDisabled']"></el-input>
				...
				<el-form-item label="保险价格" prop="insurance_price">
							  ...
							  v-if="user_group === '管理员' || (form['individual_payment_id'] && $check_field('set','insurance_price')) || (!form['individual_payment_id'] && $check_field('add','insurance_price'))" :disabled="disabledObj['insurance_price_isDisabled']"></el-input>
				...
				<el-form-item label="购买用户" prop="purchasing_users">
							...
							<!--v-if="user_group === '管理员' || (form['individual_payment_id'] && $check_field('set','purchasing_users')) || (!form['individual_payment_id'] && $check_field('add','purchasing_users'))" :disabled="disabledObj['purchasing_users_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','purchasing_users')">{{form['purchasing_users']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['individual_payment_id'] && $check_field('set','purchasing_users')) || (!form['individual_payment_id'] && $check_field('add','purchasing_users'))" id="purchasing_users" v-model="form['purchasing_users']" :disabled="disabledObj['purchasing_users_isDisabled']">
				...
				<el-form-item label="用户姓名" prop="user_name">
							  ...
							  v-if="user_group === '管理员' || (form['individual_payment_id'] && $check_field('set','user_name')) || (!form['individual_payment_id'] && $check_field('add','user_name'))" :disabled="disabledObj['user_name_isDisabled']"></el-input>
				...
				<el-form-item label="购买日期" prop="purchase_date">
								<el-date-picker :disabled="disabledObj['purchase_date_isDisabled']" v-if="user_group === '管理员' || (form['individual_payment_id'] && $check_field('set','purchase_date')) || (!form['individual_payment_id'] && $check_field('add','purchase_date'))" id="purchase_date"
				...
				<el-form-item label="备注信息" prop="remarks">
						...
						v-if="user_group === '管理员' || (form['individual_payment_id'] && $check_field('set','remarks')) || (!form['individual_payment_id'] && $check_field('add','remarks'))" :disabled="disabledObj['remarks_isDisabled']"></el-input>
	...
	export default {
		...
		data() {
			return {
				...
				url_upload: "~/api/individual_payment/upload?",
		...
		methods: {
          ...
          this.form["purchase_date"] = null;
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/individual_payment/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/individual_payment/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/individual_payment/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/individual_payment/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/individual_payment/view','get');
