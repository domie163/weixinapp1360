	...
	<el-main class="bg table_wrap">
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item label="保险名称">
									<el-input v-model="query.insurance_name"></el-input>
								</el-form-item>
				</el-col>
									<el-col :xs="24" :sm="24" :lg="8" class="el_form_search_wrap">
					<el-form-item label="保险类型">
									<el-input v-model="query.insurance_type"></el-input>
								</el-form-item>
				</el-col>
					...
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/elderly_insurance_payment/table','add') || $check_action('/elderly_insurance_payment/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            			<el-button v-if="user_group == '管理员' || $check_action('/elderly_insurance_payment/table','del') || $check_action('/elderly_insurance_payment/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
				...
				<el-table-column prop="insurance_name" @sort-change="$sortChange" label="保险名称"
				v-if="user_group == '管理员' || $check_field('get','insurance_name')" min-width="200">
					</el-table-column>
					<el-table-column prop="insurance_type" @sort-change="$sortChange" label="保险类型"
				v-if="user_group == '管理员' || $check_field('get','insurance_type')" min-width="200">
					</el-table-column>
					<el-table-column prop="insurance_price" @sort-change="$sortChange" label="保险价格"
				v-if="user_group == '管理员' || $check_field('get','insurance_price')" min-width="200">
					</el-table-column>
				...
				v-if="user_group == '管理员' || $check_field('get','user_name')" min-width="200">
					...
					<el-table-column prop="purchase_date" @sort-change="$sortChange" label="购买日期"
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/elderly_insurance_payment/table','set') || $check_action('/elderly_insurance_payment/view','set') || $check_action('/elderly_insurance_payment/view','get') || $check_action('//table','add') || $check_action('//view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/elderly_insurance_payment/table','set') || $check_action('/elderly_insurance_payment/view','set') || $check_action('/elderly_insurance_payment/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
					<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="openPayModal(scope.row)" v-if="scope.row.pay_state==='未支付' && (user_group == '管理员' || $check_pay('/elderly_insurance_payment/table'))">
						<span>支付</span>
					</el-button>
				</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/elderly_insurance_payment/get_list?like=0",
				url_del: "~/api/elderly_insurance_payment/del?",
		...
		methods: {
			...
			get_user_purchasing_users(id){
				...
				return ret;
											...
											openPayModal(v){
				this.pay_obj.id = v.elderly_insurance_payment_id
				this.payModalVisible = true
			},
			submitPay(){
				let _this = this;
				let url = this.$toUrl(this.query, "~/api/elderly_insurance_payment/set?elderly_insurance_payment_id="+this.pay_obj.id);
