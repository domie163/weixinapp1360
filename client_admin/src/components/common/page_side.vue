	...
	<el-menu class="side el-menu-vertical-demo" default-active="/" @open="handleOpen" @close="handleClose"
		...
		<el-submenu index="user"
					v-show="user_group == '管理员' || $check_group(['/user/table','/user_group/table'
						,'/elderly_users/table'
							,'/family_members_of_the_elderly/table'
			...
			<el-menu-item index="/user/table" v-show="user_group == '管理员'"><span>管理员</span></el-menu-item>
						<el-menu-item index="/elderly_users/table" v-show="user_group == '管理员' || $check_action('/elderly_users/table')"><span>老人用户</span></el-menu-item>
							<el-menu-item index="/family_members_of_the_elderly/table" v-show="user_group == '管理员' || $check_action('/family_members_of_the_elderly/table')"><span>老人家属</span></el-menu-item>
							<el-menu-item index="/service_provider/table" v-show="user_group == '管理员' || $check_action('/service_provider/table')"><span>服务商</span></el-menu-item>
				...
				for (var i = 0; i < tables.length; i++) {
					...
					if (o.path.indexOf("/table") !== -1) {
						let list = ["exam","article", "article_type", ,"comment","notice"];
							list.push("elderly_users");
								list.push("family_members_of_the_elderly");
								list.push("service_provider");
