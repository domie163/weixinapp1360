          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','corporate_name') || ($check_field('add','corporate_name') || $check_field('set','corporate_name'))" label="公司名称" name="corporate_name">
                            <uni-easyinput type="text" v-model="form['corporate_name']" v-if="user_group === '管理员' || (form['service_provider_id'] && $check_field('set','corporate_name')) || (!form['service_provider_id'] && $check_field('add','corporate_name'))" :disabled="disabledObj['corporate_name_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','corporate_name')">
                  {{ form['corporate_name'] }}
                </text>
                          </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','company_address') || ($check_field('add','company_address') || $check_field('set','company_address'))" label="公司地址" name="company_address">
                            <uni-easyinput type="text" v-model="form['company_address']" v-if="user_group === '管理员' || (form['service_provider_id'] && $check_field('set','company_address')) || (!form['service_provider_id'] && $check_field('add','company_address'))" :disabled="disabledObj['company_address_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','company_address')">
              ...
              <uni-forms-item v-if="$check_field('get','user_name') || ($check_field('add','user_name') || $check_field('set','user_name'))" label="用户姓名" name="user_name">
                            <uni-easyinput type="text" v-model="form['user_name']" v-if="user_group === '管理员' || (form['service_provider_id'] && $check_field('set','user_name')) || (!form['service_provider_id'] && $check_field('add','user_name'))" :disabled="disabledObj['user_name_isDisabled']" />
                <!-- 仅查看 -->
...
export default {
  ...
  data() {
    return {
      ...
      group_table: "service_provider",
      is_password: true,
      field: "service_provider_id",
      url_add: "~/api/service_provider/add?",
      url_set: "~/api/service_provider/set?",
      url_get_obj: "~/api/service_provider/get_obj?",
      url_upload: "~/api/service_provider/upload?",
  ...
  methods: {
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/service_provider/upload?'),
    ...
    submit_sub(form_sub){
      if(form_sub["service_provider_id"]){
        // 提交事件
        this.$post("~/api/" + this.group_table + "/set?service_provider_id=" + form_sub["service_provider_id"],form_sub,(res)=>{
    ...
    is_view(){
      ...
      if(!bl){
        bl = this.$check_action('/service_provider/table','add');
        console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
      }
      if(!bl){
        bl = this.$check_action('/service_provider/table','set');
        console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
      }
      if(!bl){
        bl = this.$check_action('/service_provider/view','add');
        console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
      }
      if(!bl){
        bl = this.$check_action('/service_provider/view','set');
        console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
      }
      if(!bl){
        bl = this.$check_action('/service_provider/view','get');
