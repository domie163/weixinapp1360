<template>
  <view class="page_service_classification diy_table diy_table_tml" id="service_classification_table">
    <!-- 筛选模块(开始) -->
    <view>
            ...
            <view class="">
              ...
              <uni-forms :modelValue="query">
                  <uni-forms-item label="服务类型" name="service_type">
                      <uni-easyinput type="text" v-model="query.service_type" placeholder="服务类型" />
                    </uni-forms-item>
                </uni-forms>
              <!-- /搜索栏 -->
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/service_classification/view?" v-show="user_group == '管理员' || $check_action('/service_classification/table','add') || $check_action('/service_classification/view','add')">添加</navigator>
		...
		<view class="warp">
			<view class="container">
				<view v-for="(o,i) in list" class="row">
										<view v-if="0 && $check_field('get','service_type')" class="col">
						<view class="view">
                  ...
                  <view class="col">
                    <view class="view">
                      <view class="create_time_block diy_filed diy_time">
                        ...
                        <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/service_classification/table','del') || $check_action('/service_classification/view','del')">删除</button>
                      </view>
                    </view>
                  </view>
					<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/service_classification/view?' + field + '=' + o[field]"
                               v-show="user_group == '管理员' || $check_action('/service_classification/table','set') || $check_action('/service_classification/view','set') || $check_action('/service_classification/view','get')" class="btn btn-primary">详情</navigator>
	        					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 获取数据地址
				url_get_list: "~/api/service_classification/get_list?like=0",
				url_del: "~/api/service_classification/del?",

				// 字段ID
				field: "service_classification_id",
