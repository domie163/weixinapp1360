<template>
  <view class="page_individual_payment diy_table diy_table_tml" id="individual_payment_table">
    <!-- 筛选模块(开始) -->
    <view>
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/individual_payment/view?" v-show="user_group == '管理员' || $check_action('/individual_payment/table','add') || $check_action('/individual_payment/view','add')">添加</navigator>
                  ...
                  <view class="col">
                    <view class="view">
                      <view class="create_time_block diy_filed diy_time">
                        ...
                        <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/individual_payment/table','del') || $check_action('/individual_payment/view','del')">删除</button>
                      </view>
                    </view>
                  </view>
					<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/individual_payment/view?' + field + '=' + o[field]"
                               v-show="user_group == '管理员' || $check_action('/individual_payment/table','set') || $check_action('/individual_payment/view','set') || $check_action('/individual_payment/view','get')" class="btn btn-primary">详情</navigator>
	                    <a @click="goto_pay(o)" size="mini"  v-if="o.pay_state==='未支付' && $check_pay('/individual_payment/table')" class='btn btn-primary'>支付</a>
	        					</view>
				</view>
			</view>
		</view>
          ...
          submit_pay(){
            ...
            this.$post(url,param,(res)=>{
              if(res.result){
                _this.$toast("支付成功");
                setTimeout(()=>{
                  _this.$nav("/pages/individual_payment/table");
                }, 700);
              }else if(res.error){
