<template>
  <view class="page_elderly_insurance_payment diy_table diy_table_tml" id="elderly_insurance_payment_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <uni-forms :modelValue="query">
                  <uni-forms-item label="保险名称" name="insurance_name">
                      <uni-easyinput type="text" v-model="query.insurance_name" placeholder="保险名称" />
                    </uni-forms-item>
                    <uni-forms-item label="保险类型" name="insurance_type">
                      <uni-easyinput type="text" v-model="query.insurance_type" placeholder="保险类型" />
                    </uni-forms-item>
                          </uni-forms>
              <!-- /搜索栏 -->
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/elderly_insurance_payment/view?" v-show="user_group == '管理员' || $check_action('/elderly_insurance_payment/table','add') || $check_action('/elderly_insurance_payment/view','add')">添加</navigator>
		...
		<view class="warp">
			<view class="container">
				<view v-for="(o,i) in list" class="row">
										<view v-if="1 && $check_field('get','insurance_name')" class="col">
						<view class="view">
                            ...
                            <span>保险名称</span>
								...
								<text>
									{{ o["insurance_name"] }}
													...
													<view v-if="1 && $check_field('get','insurance_type')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>保险类型</span>
                          </view>
								...
								<text>
									{{ o["insurance_type"] }}
													...
													<view v-if="1 && $check_field('get','insurance_price')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>保险价格</span>
                          </view>
								...
								<text>
									{{ o["insurance_price"] }}
													...
													<view v-if="0 && $check_field('get','purchasing_users')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>购买用户</span>
                          </view>
								...
								<text>
									{{ get_user_purchasing_users(o['purchasing_users']) }}
								</text>
							</view>
										</view>
					</view>
                            ...
                            <span>用户姓名</span>
								...
								<text>
									{{ o["user_name"] }}
													...
													<view v-if="0 && $check_field('get','purchase_date')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>购买日期</span>
                          </view>
                  ...
                  <view class="col">
                    <view class="view">
                      <view class="create_time_block diy_filed diy_time">
                        ...
                        <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/elderly_insurance_payment/table','del') || $check_action('/elderly_insurance_payment/view','del')">删除</button>
                      </view>
                    </view>
                  </view>
					<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/elderly_insurance_payment/view?' + field + '=' + o[field]"
                               v-show="user_group == '管理员' || $check_action('/elderly_insurance_payment/table','set') || $check_action('/elderly_insurance_payment/view','set') || $check_action('/elderly_insurance_payment/view','get')" class="btn btn-primary">详情</navigator>
	                    <a @click="goto_pay(o)" size="mini"  v-if="o.pay_state==='未支付' && $check_pay('/elderly_insurance_payment/table')" class='btn btn-primary'>支付</a>
	        					</view>
				</view>
			</view>
		</view>
          ...
          submit_pay(){
            ...
            this.$post(url,param,(res)=>{
              if(res.result){
                _this.$toast("支付成功");
                setTimeout(()=>{
                  _this.$nav("/pages/elderly_insurance_payment/table");
                }, 700);
              }else if(res.error){
