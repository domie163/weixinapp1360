<template>
  <view id="home" class="home--new">
    ...
    <Card
      class="list_diy"
      title="养老保险推荐"
      url="/pages/endowment_insurance/list"
      :list="list_article"
      v-if="$check_action('/endowment_insurance/list', 'get')"
    >
	  <list_endowment_insurance :list="list_endowment_insurance"></list_endowment_insurance>
...
<script>
						...
						import list_endowment_insurance from "@/components/diy/list_endowment_insurance.vue";
  ...
  methods: {
    ...
    toToggle() {
      this.isAdmin = !this.isAdmin;
		...
		*  获取养老保险
		*/
		get_endowment_insurance() {
			let url = "~/api/endowment_insurance/get_list?";
									
			this.$get(url, {
											}, (json) => {
					if (json.result && json.result.list) {
					console.log("养老保险" ,json.result.list);
					this.list_endowment_insurance = json.result.list;
											this.list_endowment_insurance.map((o)=>{
							o["praise_len"];
						});
						this.get_praise(this.list_endowment_insurance ,"endowment_insurance" ,"37809");
									}
			});
		},
