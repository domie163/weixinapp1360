<template>
  <view class="page_life_services diy_table diy_table_tml" id="life_services_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <uni-forms :modelValue="query">
                    <uni-forms-item label="公司名称" name="corporate_name">
                      <uni-easyinput type="text" v-model="query.corporate_name" placeholder="公司名称" />
                    </uni-forms-item>
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/life_services/view?" v-show="user_group == '管理员' || $check_action('/life_services/table','add') || $check_action('/life_services/view','add')">添加</navigator>
	...
	<view class="warp" >
		<view class="container">
			<view v-for="(o,i) in list" class="row box_style">
									<view class="col" v-if="0 && $check_field('get','service_provider')">
						<view class="view">
							<view class="diy_title" v-if="true">
								...
								<span>
									{{ get_user_service_provider(o['service_provider']) }}
								</span>
								</view>
													</view>
					</view>
									<view class="col" v-if="1 && $check_field('get','corporate_name')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>公司名称</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="1 && $check_field('get','service_name')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>服务名称</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="1 && $check_field('get','service_type')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>服务类型</span>
							</view>
						...
						<view class="view">
							<view class="diy_title" v-if=" false">
								<span>封面图片</span>
									...
									<view class="col" v-if="1 && $check_field('get','service_price')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>服务价格</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','service_details')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>服务详情</span>
							</view>
				...
				<view class="col">
					<view class="view">
						<view class="create_time_block diy_filed diy_time">
                          ...
                          <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/life_services/table','del') || $check_action('/life_services/view','del')">删除</button>
                        </view>
					</view>
				</view>
				<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/life_services/view?' + field + '=' + o[field]"
						v-show="user_group == '管理员' || $check_action('/life_services/table','set') || $check_action('/life_services/view','set') || $check_action('/life_services/view','get')" class="btn btn-primary">详情</navigator>
					                    																		<!--跨表按钮-->
															<a type="button" size="mini" class="btn btn-primary" v-if="$check_action('/appointment_order/view','add')" @click="to_form('/pages/appointment_order/view',o)">
									<span>预约</span>
								</a>
																						</view>
			</view>
