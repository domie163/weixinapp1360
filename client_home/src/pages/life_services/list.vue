<template>
	<view class="page_life_services diy_list diy_list_page div_list_tml" id="life_services_list">
		<!-- 筛选模块(开始) -->
		<view>
			<view class="container">
				<view>
					<view>
						<view class="">
							<!-- 搜索栏 -->
														<uni-search-bar placeholder="搜索公司名称" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'corporate_name')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
																			<uni-search-bar placeholder="搜索服务名称" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'service_name')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
																				<!-- /搜索栏 -->
						</view>
				...
				<view>
					<view>
						<view class="tab_view">
							<!-- 筛选栏 -->
																						<uni-section title="服务类型" type="line">
							<uni-data-select
									@change="change_service_type"
			...
			<view class="container">
				<view v-for="(o,i) in list" class="row box_style">
					...
					<view class="uni-padding-wrap uni-common-mt">
						<navigator :url="'./details?life_services_id=' + o['life_services_id']" class="btn btn-primary">查看详情</navigator>
...
<script>
	 ...
	 */
	get_praise(list){
		this.$get("~/api/praise/count_group?groupby=source_id&source_table=life_services", {}, (res) => {
			if(res.result){
				res.result.map((o)=>{
					for(var i = 0;i < list.length; i++){
						var oj = list[i];
						if(oj["life_services_id"] === o["source_id"]){
