          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','user_name') || ($check_field('add','user_name') || $check_field('set','user_name'))" label="用户姓名" name="user_name">
                            <uni-easyinput type="text" v-model="form['user_name']" v-if="user_group === '管理员' || (form['family_members_of_the_elderly_id'] && $check_field('set','user_name')) || (!form['family_members_of_the_elderly_id'] && $check_field('add','user_name'))" :disabled="disabledObj['user_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','user_gender') || ($check_field('add','user_gender') || $check_field('set','user_gender'))" label="用户性别" name="user_gender">
                  ...
                  v-if="user_group === '管理员' || (form['family_members_of_the_elderly_id'] && $check_field('set','user_gender')) || (!form['family_members_of_the_elderly_id'] && $check_field('add','user_gender'))"
                ></uni-data-select>
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','user_age') || ($check_field('add','user_age') || $check_field('set','user_age'))" label="用户年龄" name="user_age">
                            <uni-easyinput type="text" v-model="form['user_age']" v-if="user_group === '管理员' || (form['family_members_of_the_elderly_id'] && $check_field('set','user_age')) || (!form['family_members_of_the_elderly_id'] && $check_field('add','user_age'))" :disabled="disabledObj['user_age_isDisabled']" />
                <!-- 仅查看 -->
...
export default {
  ...
  data() {
    return {
      ...
      group_table: "family_members_of_the_elderly",
      is_password: true,
      field: "family_members_of_the_elderly_id",
      url_add: "~/api/family_members_of_the_elderly/add?",
      url_set: "~/api/family_members_of_the_elderly/set?",
      url_get_obj: "~/api/family_members_of_the_elderly/get_obj?",
      url_upload: "~/api/family_members_of_the_elderly/upload?",

      query: {
        "family_members_of_the_elderly_id": 0,
      },
      ...
      form: {
            "user_name":  '', // 用户姓名
                    ...
                    "user_age":  '', // 用户年龄
                        "examine_state": "未审核",
                        "user_id": 0,
                "family_members_of_the_elderly_id": 0, // ID
  ...
  methods: {
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/family_members_of_the_elderly/upload?'),
    ...
    submit_sub(form_sub){
      if(form_sub["family_members_of_the_elderly_id"]){
        // 提交事件
        this.$post("~/api/" + this.group_table + "/set?family_members_of_the_elderly_id=" + form_sub["family_members_of_the_elderly_id"],form_sub,(res)=>{
    ...
    is_view(){
      ...
      if(!bl){
        bl = this.$check_action('/family_members_of_the_elderly/table','add');
        console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
      }
      if(!bl){
        bl = this.$check_action('/family_members_of_the_elderly/table','set');
        console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
      }
      if(!bl){
        bl = this.$check_action('/family_members_of_the_elderly/view','add');
        console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
      }
      if(!bl){
        bl = this.$check_action('/family_members_of_the_elderly/view','set');
        console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
      }
      if(!bl){
        bl = this.$check_action('/family_members_of_the_elderly/view','get');
