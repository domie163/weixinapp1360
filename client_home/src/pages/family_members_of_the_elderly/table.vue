<template>
  <view class="page_family_members_of_the_elderly diy_table diy_table_tml" id="family_members_of_the_elderly_table">
    <!-- 筛选模块(开始) -->
    <view>
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/family_members_of_the_elderly/view?" v-show="user_group == '管理员' || $check_action('/family_members_of_the_elderly/table','add') || $check_action('/family_members_of_the_elderly/view','add')">添加</navigator>
		...
		<view class="warp">
			<view class="container">
				<view v-for="(o,i) in list" class="row">
										<view v-if="1 && $check_field('get','user_name')" class="col">
						<view class="view">
                            ...
                            <span>用户姓名</span>
								...
								<text>
									{{ o["user_name"] }}
													...
													<view v-if="1 && $check_field('get','user_gender')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>用户性别</span>
                          </view>
								...
								<text>
									{{ o["user_gender"] }}
													...
													<view v-if="1 && $check_field('get','user_age')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>用户年龄</span>
                          </view>
                  ...
                  <view class="col">
                    <view class="view">
                      <view class="create_time_block diy_filed diy_time">
                        ...
                        <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/family_members_of_the_elderly/table','del') || $check_action('/family_members_of_the_elderly/view','del')">删除</button>
                      </view>
                    </view>
                  </view>
					<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/family_members_of_the_elderly/view?' + field + '=' + o[field]"
                               v-show="user_group == '管理员' || $check_action('/family_members_of_the_elderly/table','set') || $check_action('/family_members_of_the_elderly/view','set') || $check_action('/family_members_of_the_elderly/view','get')" class="btn btn-primary">详情</navigator>
	        					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 获取数据地址
				url_get_list: "~/api/family_members_of_the_elderly/get_list?like=0",
				url_del: "~/api/family_members_of_the_elderly/del?",
